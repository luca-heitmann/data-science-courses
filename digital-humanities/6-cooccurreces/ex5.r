...
sotu_corpus <- corpus(textdata$Paragraph,
  docnames = textdata$X,
  docvars = data.frame(
    year = substr(textdata$Date, 0, 4)
  )
)

# original corpus length and its first document
ndoc(sotu_corpus)
substr(as.character(sotu_corpus)[1], 0, 200)
...
binDTM_1 <- binDTM[docvars(corpus_sentences)$year < "1960", ]
binDTM_2 <- binDTM[docvars(corpus_sentences)$year >= "1960", ]


# Read in the source code for the co-occurrence calculation
source("6-cooccurreces/calculateCoocStatistics.R")
# Definition of a parameter for the representation
# of the co-occurrences of a concept
numberOfCoocs <- 15
...
for (i in 1:numberOfCoocs) {
  # Calling up the co-occurrence calculation for
  # term i from the search words co-occurrences
...
V(graphNetwork)$color <- ifelse(V(graphNetwork)$name == coocTerm,
...
par(mai = c(0, 0, 1, 0))
# Final Plot
png(filename = "6-cooccurreces/railway_cooccurrence.png", width = 1600, height = 1200, res = 150)
plot(
  graphNetwork,
