...
def get_model_components(model_type, lora_r, lora_alpha, lora_dropout):
    ...
    if model_type == "gpt2":
        ...
        def preprocess_function_gpt2_luca(examples, tokenizer):
            ...
            full_instruction_prompts = [
                create_prompt_gpt2({"dialogue": dialogue, "summary": summary}) + tokenizer.eos_token
        ...
        def preprocess_function_gpt2(examples, tokenizer):
            # 1. Full Prompt erstellen
            full_instruction_prompts = [
                create_prompt_gpt2({"dialogue": d, "summary": s}) + tokenizer.eos_token
            ...
            # Wir loopen durch jedes Beispiel, um die Labels einzeln zu maskieren
            for i in range(len(full_instruction_prompts)):
                input_ids = tokenized_outputs["input_ids"][i]
                
                # Erstelle den Prompt OHNE Summary, um die Länge zu bestimmen
                prompt_only = create_prompt_gpt2({"dialogue": examples["dialogue"][i]})
                # WICHTIG: Keine speziellen Tokens hinzufügen, um die exakte Länge zu messen
                prompt_tokenized = tokenizer(prompt_only, truncation=True, max_length=256)
                prompt_length = len(prompt_tokenized["input_ids"])

                # Label Kopie erstellen
                label = list(input_ids) 
                
                # 1. Alles VOR der Summary auf -100 setzen (Prompt-Maskierung)
                for j in range(prompt_length):
                    label[j] = -100
                    
                # 2. Alles nach dem EOS-Token oder im Padding auf -100 setzen
                # Das erledigen wir über die Attention Mask
...
def main(args):
    ...
    if args.model_type == "gpt2":
        ...
        generated_summary = tokenizer.decode(outputs[0][input_token_count:], skip_special_tokens=True)
        print(f"\n**Generated Summary:**\n{generated_summary}")
    else:
