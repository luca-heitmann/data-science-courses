...
class EuroSatMsDataset(Dataset):
    def __init__(self, dataset_path, split_file, transform=None):
        ...
        # fixed 6 channels for task 3 (B04,B03,B02,B05,B08,B12)
        # bands = {
        #        'B01': 'Coastal Aerosol',
        #        'B02': 'Blue',
        #        'B03': 'Green',
    ...
    def __getitem__(self, idx):
        ...
        image = imread(img_path)                # (H,W,13)
        image = image[:, :, self.channels]      # (H,W,6)
        image = image.astype(np.float32) / 65535.0
        image = torch.from_numpy(image).permute(2, 0, 1)  # (6,H,W)
